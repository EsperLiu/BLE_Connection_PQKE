...
void shake128_inc_init(shake128incctx *state) {
    state->ctx = k_malloc(PQC_SHAKEINCCTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
void shake128_inc_ctx_clone(shake128incctx *dest, const shake128incctx *src) {
    dest->ctx = k_malloc(PQC_SHAKEINCCTX_BYTES);
    if (dest->ctx == NULL) {
        exit(111);
    }
...
void shake128_absorb(shake128ctx *state, const uint8_t *input, size_t inlen) {
    state->ctx = k_malloc(PQC_SHAKECTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
void shake128_ctx_clone(shake128ctx *dest, const shake128ctx *src) {
    dest->ctx = k_malloc(PQC_SHAKECTX_BYTES);
    if (dest->ctx == NULL) {
        exit(111);
    }
